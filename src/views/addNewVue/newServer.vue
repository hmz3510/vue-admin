    ...
    <div class="formCss">
      ...
      >
        <el-form-item label="监控类别" prop="watchType">
          <el-select v-model="ruleForm.watchType" placeholder="应用服务进程" style="min-width:240px">
            <el-option label="应用服务进程" value="1" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="监控参数" prop="watchNumber">
          <el-select v-model="ruleForm.watchNumber" style="min-width:240px">
            <el-option
              v-for="item in watchNumberLst"
              :key="item.id"
              :label="item.label"
              :value="item.id"
            />
  ...
  data() {
    return {
      ...
      rules: {
        ...
        watchNumber: [
          { required: true, message: '请选择监控参数', trigger: 'change' }
        ]
      },
      watchNumberLst: [{
        id: 1,
